...
public function insertData($sku, $name, $price, $size, $height, $width, $length, $weight) {
    // Check if SKU already exists in database
        $stmt = $this->conn->prepare("SELECT COUNT(*) FROM twopagestest WHERE sku = :sku");
    ...
    try {
        $stmt = $this->conn->prepare("INSERT INTO twopagestest (sku, name, price, size, height, width, length, weight) VALUES (:sku, :name, :price, :size, :height, :width, :length, :weight)");
        $stmt->bindParam(':sku', $sku);
    ...
    public function getData() {
        $stmt = $this->conn->prepare("SELECT * FROM twopagestest");
        if ($stmt->execute()) {
    ...
    public function deleteProductBtn($deleteCheckbox) {
        try {
            ...
            $sql = "DELETE FROM twopagestest WHERE id IN ($placeholders)";
